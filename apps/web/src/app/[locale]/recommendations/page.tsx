...
function formatScore(val: number | null, locale: string) {
...
function formatPrice(val: number | null, locale: string) {
  if (val === null || val === undefined) return "—";
  return new Intl.NumberFormat(locale, {
    style: "currency",
    currency: "CNY",
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex items-center gap-3 md:gap-4 flex-wrap md:flex-nowrap shrink-0">
            <div className="flex flex-wrap gap-2">
              <ScoreBadge
                label={t("score")}
                value={formatScore(rec.final_score, locale)}
                icon={TrendingUp}
              />
              <ScoreBadge
                label={t("quantScore")}
                value={formatScore(rec.quant_score, locale)}
                icon={BarChart3}
              />
              <ScoreBadge
                label={t("catalystScore")}
                value={formatScore(rec.catalyst_score, locale)}
                icon={Zap}
              />
            </div>
            <div className="text-right hidden sm:block">
              ...
              <p className="text-base font-medium text-ink tabular-nums">
                {formatPrice(rec.price_at_rec, locale)}
              </p>
            </div>
          ...
          <div className="pt-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-6">
              <div>
                ...
                {formatReturn(rec.return_t1, locale, t)}
              </div>
              <div>
                <span className="text-xs text-warm-gray block">{t("returnT5")}</span>
                {formatReturn(rec.return_t5, locale, t)}
              </div>
            </div>
        ...
        <div key={dayGroup.rec_date} className="bg-white border border-divider rounded-xl overflow-hidden">
          <div className="px-4 md:px-6 py-3 border-b border-divider bg-cream/30 flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-charcoal">
              {formatDate(dayGroup.rec_date, locale)}
            </span>
                ...
                {dayGroup.recommendations.map((rec) => (
                  <tr key={rec.id} className="hover:bg-cream/30 transition-colors">
                    ...
                    <td className="px-6 py-3 text-right font-medium text-ink tabular-nums">
                      {formatScore(rec.final_score, locale)}
                    </td>
          ...
          <div className="md:hidden divide-y divide-cream-dark">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium tabular-nums">{formatScore(rec.final_score, locale)}</span>
                </div>
